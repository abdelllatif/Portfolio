  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      try {
        ...
        if (!userData.public_repos || typeof userData.followers !== 'number' || typeof userData.following !== 'number') {
        ...
        if (!Array.isArray(reposData)) {
          throw new Error("Invalid repositories data received from GitHub API");
        }
        ...
        try {
          ...
          if (contributionsResponse.ok) {
            ...
            if (contributionsData.errors) {
              console.warn("GraphQL errors:", contributionsData.errors);
            } else if (contributionsData.data && contributionsData.data.user) {
              totalContributions =
                contributionsData.data.user.contributionsCollection
                  .contributionCalendar.totalContributions;
            }
          } else {
            console.warn(`Contributions API failed: ${contributionsResponse.status} ${contributionsResponse.statusText}`);
          }
        } catch (error) {
          console.warn(
        ...
        setError(err.message || "Failed to load GitHub data. Please check your token and username.");
      } finally {
        setLoading(false);
      }
    };
